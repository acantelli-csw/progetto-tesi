...
def decide_tools(prompt: str) -> dict:
    ...
    examples = [
        {
            "prompt": "Come funziona il processo di approvazione degli ordini in SAM ERP2?",
            "decision": {
                "use_semantic": True,
                "use_keyword": False,
                "reason": "Richiesta su un workflow generale di SAM ERP2. Non ci sono termini precisi da cercare — la ricerca semantica trova documenti con concetti correlati al processo di approvazione."
            }
        },
        {
            "prompt": "Qual è l'email di riferimento per il cliente GMR Enlights?",
            "decision": {
                "use_semantic": False,
                "use_keyword": True,
                "reason": "Richiesta su un dato specifico legato a un cliente preciso. La keyword search trova direttamente le RI che menzionano 'GMR Enlights' molto più efficacemente della ricerca semantica."
            }
        },
        {
            "prompt": "Implementazione del modulo paghe con tracciato personalizzato per azienda manifatturiera",
            "decision": {
                "use_semantic": True,
                "use_keyword": True,
                "reason": "Richiesta su una personalizzazione complessa: la ricerca semantica trova RI con casistiche simili di paghe, quella keyword trova RI che menzionano esplicitamente 'tracciato' o il settore specifico."
            }
        },
        {
            "prompt": "Quali sono le migliori tecniche di project management agile?",
            "decision": {
                "use_semantic": False,
                "use_keyword": False,
                "reason": "La richiesta non riguarda SAM ERP2 né le sue personalizzazioni. Le RI non contengono informazioni su metodologie di project management generali."
            }
        }
    ]
    ...
    Regole di decisione (in ordine di priorità):

    1. Se la richiesta è FUORI DAL DOMINIO ERP (non riguarda SAM ERP2 né le sue personalizzazioni):
    → use_semantic: False, use_keyword: False

    2. Se la richiesta contiene TERMINI PRECISI e IDENTIFICATIVI (nome cliente, nome modulo specifico,
    codice RI, nome funzionalità con termine tecnico esatto):
    → use_semantic: False, use_keyword: True

    3. Se la richiesta riguarda CONCETTI GENERALI, WORKFLOW o FUNZIONALITÀ descritte in modo generico
    senza un termine tecnico preciso da cercare:
    → use_semantic: True, use_keyword: False

    4. Solo se la richiesta combina PERSONALIZZAZIONI COMPLESSE dove servono sia concetti correlati
    sia riferimenti precisi, oppure se è genuinamente ambigua tra caso 2 e caso 3:
    → use_semantic: True, use_keyword: True

    La scelta both (caso 4) è la più costosa — usala solo quando necessaria, non come default.

    Rispondi SEMPRE in formato JSON:
    {{
        "use_semantic": true/false,
        "use_keyword": true/false,
        "reason": "<quale regola si applica e perché>"
    }}
...
def select_documents(user_prompt: str, chunks: list) -> dict:
    ...
    Nota:
    - La ricerca documentale restituisce sempre tutti i documenti più rilevanti, ma non tutti sono effettivamente utili.
...
def generate_final_answer(user_prompt: str, selected_docs: list, chat_history: list = None) -> str:
    ...
    Regole:
    1. Se i documenti forniti non contengono un'informazione specifica e diretta che risponde alla domanda, dichiaralo esplicitamente scrivendo che l'informazione non è disponibile nella documentazione. Non inferire, non generalizzare e non rispondere basandoti su conoscenze esterne ai documenti.
