...
con utilizzo di LLM-as-judge tramite API OpenAI.
...
# Import per OpenAI e embeddings
try:
    from openai import OpenAI
    import tiktoken
except ImportError:
    print("ATTENZIONE: Installare openai")
    print("Aggiungi al pyproject.toml: openai = \"^1.0.0\"")
    print("Poi esegui: uv sync")
...
def get_embedding(text: str, client: OpenAI, model: str = "text-embedding-3-large") -> List[float]:
    """
    Ottiene l'embedding di un testo tramite API OpenAI.
    
    Args:
        text: Testo da embeddare
        client: Client OpenAI
        model: Modello di embedding (text-embedding-ada-002 o text-embedding-3-large)
...
def evaluate_retrieval_for_query(
    ...
    Args:
        ...
        client: Client OpenAI per LLM-as-judge
...
def evaluate_semantic_similarity_with_embeddings(
    ...
    try:
        emb_generated = get_embedding(generated_answer, client, embedding_model)
        emb_reference = get_embedding(reference_answer, client, embedding_model)
...
def main():
    ...
    # 1. Configurazione
    print("\n[1/5] Configurazione OpenAI client...")
